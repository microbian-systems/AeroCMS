...
The generator should also emit a generated helper for Marten subclass mapping. Crucially, it must explicitly bind the `Type` to the exact string discriminator discovered from the `[BlockMetadata]` attribute. If you just pass an array of types to Marten's `AddSubClassHierarchy`, it defaults to using the CLR type name (e.g., `"AeroHeroBlock"`) rather than the JSON discriminator (e.g., `"aero_hero"`), causing divergence and querying issues.

```csharp
// Auto-generated by Aero.Cms.SourceGenerators
public static partial class GeneratedBlockMartenConfiguration
{
    public static void ConfigureBlocks(Marten.StoreOptions opts)
    {
        opts.Schema.For<BlockBase>()
            .AddSubClass<AeroHeroBlock>("aero_hero")
            .AddSubClass<RichTextBlock>("rich_text")
            .AddSubClass<NavigationBlock>("navigation");
            // ... generated dynamically at compile time
    }
}
```

The hand-written Marten configuration can then simply call this generated method:

```csharp
GeneratedBlockMartenConfiguration.ConfigureBlocks(opts);
```

### Automating Polymorphic Serialization

Currently, `BlockBase.cs` contains dozens of manually maintained `[JsonDerivedType]` attributes. This is a major point of drift. 

To eliminate this manual registration entirely, `BlockBase` should be declared as a `partial class`:

```csharp
// BlockBase.cs (Manually authored)
...
The Source Generator will then automatically discover all blocks and emit a `BlockBase.g.cs` file that applies the `[JsonDerivedType]` attributes directly to the base class at compile time:

```csharp
// BlockBase.g.cs (Auto-generated by Aero.Cms.SourceGenerators)
using System.Text.Json.Serialization;
...
*(Note: For this to work, the `Aero.Cms.Abstractions` project must reference the `Aero.Cms.SourceGenerators` analyzer project).*

With this change, the workflow to add a new block is truly reduced to just two files: creating the Block Model, and creating the tagged Block Renderer. Everything else is generated.
...
To prevent state leakage across multiple browser tabs, the receiver **must not** use a static `[JSInvokable]` method. Instead, the Blazor component should pass a scoped `DotNetObjectReference` to JavaScript.

**C# Component (`Preview.cshtml.cs`):**
```csharp
private DotNetObjectReference<PreviewPageModel>? _componentRef;

protected override void OnInitialized()
{
    _componentRef = DotNetObjectReference.Create(this);
    // Pass this reference to your JS interop initialization...
}

[JSInvokable]
public void UpdatePreviewState(string payload)
{
    // Deserialize the JSON payload and update THIS instance's draft state.
    // StateHasChanged() only affects THIS preview.
    StateHasChanged();
}

public void Dispose()
{
    _componentRef?.Dispose(); // Prevent memory leaks
}
```

**JavaScript:**
```javascript
// Received from Blazor during initialization
const componentRef = /* the DotNetObjectReference */;
...
window.addEventListener('message', (event) => {
    ...
    if (event.data?.type === 'AERO_UPDATE_PREVIEW') {
        // Use the scoped instance reference, NOT a static assembly invoke
        componentRef.invokeMethodAsync('UpdatePreviewState', event.data.payload);
    }
});
```

### Option B: Static SSR Fragment Preview (Recommended Approach)

**Because the Aero Manager is a Blazor WebAssembly / .NET MAUI hybrid application**, this is the highly recommended approach. The Manager application runs on the client, making interactive Server-Side Blazor SignalR connections (Option A) unnecessarily heavy and complex to bridge.

Instead, the client-side Manager should use debounced server preview updates via HTTP:
...
3. The server renders the same layout and block adapters using `HtmlRenderer` (or a dedicated Minimal API endpoint).
4. The iframe or preview panel in the WASM/MAUI client receives and displays the returned HTML fragment.

This option is slightly less instant than an interactive iframe, but it is entirely stateless on the server, eliminates SignalR connection overhead, is significantly simpler to secure, and provides a 100% Native AOT-compatible rendering path.
