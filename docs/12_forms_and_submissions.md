...
### Data Persistence via AeroDB.Sable

When a form is submitted, the data should be persisted as a `FormSubmission` document in AeroDB.Sable (SurrealDB). Since Sable is a document store, all submissions are typically stored in the same collection (table), but they are differentiated by a `FormName` or `BlockId` property.
...
Because AeroDB.Sable supports LINQ and JSON-path queries, you can easily filter submissions by form:
