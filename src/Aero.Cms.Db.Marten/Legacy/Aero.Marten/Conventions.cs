...
{
    /// <summary>
    /// The prefix used for compare/exchange values used by AeroDB.Identity to ensure user uniqueness based on email address.
    /// </summary>
    ...
    {
        // Aero has special handling for consecutive IDs (AppUsers/1)
        if (idType == UserIdType.Consecutive)
        {
            return CollectionNameFor<TUser>(db) + "|"; // pipe character convention: https://Aerodb.net/docs/article-page/4.2/csharp/client-api/document-identifiers/working-with-document-identifiers#identities
        }

        // Aero has special handling for number-tag IDs (AppUsers/1-a)
        if (idType == UserIdType.NumberTag)
        {
            return null; // null ID for autogenerated IDs: https://Aerodb.net/docs/article-page/4.2/csharp/client-api/document-identifiers/working-with-document-identifiers#autogenerated-ids
        }
    ...
    /// <summary>
    /// Creates a user ID using the Aero collection name of the specified <typeparamref name="TUser"/> and configured identity parts separator.
    ...
    /// <param name="db">The Aero database. Used for determining the identity parts separator and collection name.</param>
    ...
    /// <param name="db">The Aero database. Used for finding the collection name for <typeparamref name="TRole"/>s and the identity parts separator.</param>
