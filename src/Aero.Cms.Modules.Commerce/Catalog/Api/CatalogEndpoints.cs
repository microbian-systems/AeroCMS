...
public sealed record ProductRequest(string Name, string? Description, string Sku, int StockQuantity, bool IsActive, Dictionary<string, string>? Attributes, List<string>? Tags, ProductFulfillmentMode FulfillmentMode = ProductFulfillmentMode.Inventory, long Version = 0)
{ public ProductDocument ToDocument() => new() { Name = Name, Description = Description, Sku = Sku, FulfillmentMode = FulfillmentMode, StockQuantity = StockQuantity, IsActive = IsActive, Attributes = Attributes ?? [], Tags = Tags ?? [], Version = Version }; }

/// <summary>Manager-owned listing input. Site and tenant ownership are server-derived.</summary>
public sealed record ListingRequest(long ProductId, string Culture, string Slug, string Name, string? ShortDescription, string? Description, string? Category, string? ImageUrl, decimal Price, decimal? CompareAtPrice, bool IsPublished, bool IsFeatured, SubscriptionOffer? SubscriptionOffer = null, long Version = 0, bool IncludeInSearch = true, bool IncludeInPublicAi = false)
{ public ProductListingDocument ToDocument() => new() { ProductId = ProductId, Culture = Culture, Slug = Slug, Name = Name, ShortDescription = ShortDescription, Description = Description, Category = Category, ImageUrl = ImageUrl, Price = Price, CompareAtPrice = CompareAtPrice, SubscriptionOffer = SubscriptionOffer, IsPublished = IsPublished, IsFeatured = IsFeatured, IncludeInSearch = IncludeInSearch, IncludeInPublicAi = IncludeInPublicAi, Version = Version }; }

public sealed record CatalogManagerPage<T>(IReadOnlyList<T> Items, long TotalCount);
public sealed record ManagerProductResponse(long Id, string Name, string? Description, string Sku, int StockQuantity, bool IsActive, IReadOnlyDictionary<string, string> Attributes, IReadOnlyList<string> Tags, ProductFulfillmentMode FulfillmentMode, long Version)
{ public static ManagerProductResponse From(ProductDocument product) => new(product.Id, product.Name, product.Description, product.Sku, product.StockQuantity, product.IsActive, product.Attributes, product.Tags, product.FulfillmentMode, product.Version); }
public sealed record ManagerListingResponse(long Id, long ProductId, string Culture, string Slug, string Name, string? ShortDescription, string? Description, string? Category, string? ImageUrl, decimal Price, decimal? CompareAtPrice, string Currency, bool IsPublished, bool IsFeatured, SubscriptionOffer? SubscriptionOffer, long Version, bool IncludeInSearch, bool IncludeInPublicAi)
{ public static ManagerListingResponse From(ProductListingDocument listing) => new(listing.Id, listing.ProductId, listing.Culture, listing.Slug, listing.Name, listing.ShortDescription, listing.Description, listing.Category, listing.ImageUrl, listing.Price, listing.CompareAtPrice, listing.Currency, listing.IsPublished, listing.IsFeatured, listing.SubscriptionOffer, listing.Version, listing.IncludeInSearch, listing.IncludeInPublicAi); }
