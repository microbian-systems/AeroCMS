...
public sealed class AeroPostGrain : AeroActor, IAeroPostActor
{
        ...
        long siteId, int skip, int take, string? search, CancellationToken ct)
    {
        ...
        var query = session.Query<BlogPostDocument>().Where(x => x.SiteId == siteId);
    ...
    public async Task<PostViewModel?> LoadAsync(long id, long siteId, CancellationToken ct)
    {
        ...
        var cached = await TryGetCacheAsync<BlogPostDocument>(cacheKey, ct);
        if (cached is not null)
            return MapToViewModel(cached);

        var document = await session.LoadAsync<BlogPostDocument>(id, ct);
    ...
    public async Task<PostViewModel?> FindBySlugAsync(string slug, long siteId, CancellationToken ct)
    {
        ...
        var document = await session.LoadAsync<BlogPostDocument>(reservation.OwnerId, ct);
        if (document is null)
            return null;
    ...
    public async Task<AeroRequestResponse<PostViewModel>> SavePostAsync(PostViewModel vm, long siteId, CancellationToken ct)
    {
        ...
        var existingPost = await session.LoadAsync<BlogPostDocument>(post.Id, ct);
    ...
    public async Task<AeroRequestResponse<PostViewModel>> DeletePostAsync(long id, long siteId, CancellationToken ct)
    {
        ...
        var post = await session.LoadAsync<BlogPostDocument>(id, ct);
    ...
    {
        await using var session = _store.LightweightSession();
        var doc = await session.LoadAsync<BlogPostDocument>(id, ct);
    ...
    {
        await using var session = _store.LightweightSession();
        var docs = await session.Query<BlogPostDocument>()
    ...
    public async Task<AeroRequestResponse<PostViewModel>> UpdateAsync(IRequest request, CancellationToken ct)
    {
        ...
        await using var session = _store.LightweightSession();
        var existing = await session.LoadAsync<BlogPostDocument>(update.Id, ct);
    ...
    public async Task<AeroRequestResponse<PostViewModel>> DeleteAsync(IRequest request, CancellationToken ct)
    {
        ...
        await using var session = _store.LightweightSession();
        var existing = await session.LoadAsync<BlogPostDocument>(delete.Id, ct);
        if (existing is null)
    ...
    private static PostViewModel MapToViewModel(BlogPostDocument d)
    {
    ...
    private static BlogPostDocument MapToDocument(PostViewModel vm)
    {
        var doc = new BlogPostDocument
        {
            Id = vm.Id,
            SiteId = vm.SiteId,
    ...
    private Task PublishContentUpdatedAsync(BlogPostDocument post, string? oldSlug, CancellationToken ct)
        => _bus is null
            ? Task.CompletedTask
