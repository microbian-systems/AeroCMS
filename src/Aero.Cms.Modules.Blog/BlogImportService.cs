...
public sealed class BlogImportService : IBlogImportService
{
        ...
        ImportFileRequest request, CancellationToken ct = default)
    {
        try
        {
            ...
            var importedPosts = new List<BlogPostDocument>();
            ...
            for (int i = 0; i < importablePosts.Count; i++)
            {
                ...
                try
                {
                    ...
                    {
                        var existingDoc = await _session.LoadAsync<BlogPostDocument>(existingSlug.OwnerId, ct);
    ...
    private static BlogPostDocument CreateNewPost(
