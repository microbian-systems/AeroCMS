...
/// <summary>Bounds provider sign-in starts by the transport and host-resolved storefront scope.</summary>
public sealed class ExternalMemberProviderBeginRateLimiter(TimeProvider timeProvider)
{
    ...
    public bool TryAcquire(HttpContext context, ISiteContext siteContext, string trustedHost)
    {
        ArgumentNullException.ThrowIfNull(context);
        ArgumentNullException.ThrowIfNull(siteContext);
        if (siteContext.TenantId <= 0 || siteContext.SiteId <= 0 ||
            string.IsNullOrWhiteSpace(trustedHost))
        {
            return false;
        }
