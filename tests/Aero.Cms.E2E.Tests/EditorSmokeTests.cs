...
public sealed class EditorSmokeTests
{
    ...
    [Test]
    public async Task BlazorFrameworkAssetsUseMapStaticAssetsMetadata()
    {
        await Fixture.LoginAsync();
        await Fixture.WarmUpBlazorAsync();
        var page = Fixture.Page!;

        var wasmUrl = await page.EvaluateAsync<string?>(
            """
            () => performance
                .getEntriesByType('resource')
                .map(entry => entry.name)
                .find(url => /\/_framework\/dotnet\.native\..+\.wasm$/i.test(url))
                ?? null
            """);

        wasmUrl.Should().NotBeNullOrWhiteSpace(
            "the Blazor runtime should load a fingerprinted native WASM asset");
        ...
        response.Headers.TryGetValue("etag", out var etag).Should().BeTrue();
        etag.Should().Be(
            $"\"{contentHash}\"",
            "MapStaticAssets uses a content-derived ETag for build-time assets");
        ...
        cacheControl.Should().Contain(
            "no-cache",
            "the E2E host runs in Development where static-asset caching is disabled");
    }

    [Test]
    public async Task RuntimeMediaFilesBypassPublicPageCatchAllRouting()
    {
        await Fixture.LoginAsync();
        var page = Fixture.Page!;

        var fileName = $"e2e-runtime-media-{Guid.NewGuid():N}.txt";
        var mediaDirectory = Path.Combine(Fixture.WebRootPath, "media");
        var filePath = Path.Combine(mediaDirectory, fileName);
        const string expected = "runtime media";
        ...
        {
            var response = await page.APIRequest.GetAsync(
                $"{Fixture.BaseUrl}/media/{fileName}");

            response.Status.Should().Be(200);
            (await response.TextAsync()).Should().Be(expected);
        }
        finally
        {
